...
export const UpdateTaskModal = ({ id, token, isOpen, handleModal }: TaskModalProps) => {

    const { collectionId, handleTasks, taskId } = useContext(DashboardContext);
    ...
    const addCollection = async (data: FormProps) => {
        ...
        if (taskId != null && collectionId != null) {
            const response = await service.update(id, token, taskId, name, str, false).catch((error) => {
